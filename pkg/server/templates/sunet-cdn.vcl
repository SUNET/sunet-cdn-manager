...
backend haproxy_https_{{$originGroup.Name}} {
  .path = "/shared/unix-sockets/haproxy_https_{{$originGroup.Name}}";
  .proxy_header = 2;
}
{{- end}}
{{- if $originGroup.HTTP }}
backend haproxy_http_{{$originGroup.Name}} {
  .path = "/shared/unix-sockets/haproxy_http_{{$originGroup.Name}}";
  .proxy_header = 2;
}
{{- end}}
{{- end}}
...
sub vcl_recv {
  ...
  if (proxy.is_ssl()) {
    ...
    {{- if .DefaultForHTTPS}}
    set req.backend_hint = haproxy_https_{{$.DefaultOriginGroupName}};
    {{- end}}
    {{- else}}
    ...
    {{- if $.DefaultForHTTP}}
    set req.backend_hint = haproxy_http_{{$.DefaultOriginGroupName}};
    {{- end}}
    {{- else}}
